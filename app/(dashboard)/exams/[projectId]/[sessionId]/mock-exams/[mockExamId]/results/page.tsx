...
export default function MockExamResultsPage() {
  ...
  const handleExport = async () => {
    const res = await fetch(`/api/mock-exams/${params.mockExamId}/attempts?format=csv`)
    if (!res.ok) return
      ...
      {stats && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            ...
            { label: 'Average Score', value: `${stats.avgScore?.toFixed(1) || 0}%`, icon: TrendingUp },
            { label: 'Pass Rate', value: `${stats.passRate?.toFixed(1) || 0}%`, icon: Award },
          ].map((s) => {
            const Icon = s.icon
            return (
              <Card key={s.label}>
                    ...
                    return (
                      <tr key={attempt.id} className="hover:bg-gray-50">
                        <td className="p-3 font-medium">
                          {attempt.user?.name || attempt.guestName || 'Student'}
                        </td>
