  ...
  const handleCopy = async () => {
    ...
    try {
      const res = await fetch(`/api/sessions/${sourceSession.id}/copy`, {
        ...
        body: JSON.stringify({ targetSessionId: targetId, copySections, copyTopics }),
      })
      const data = await res.json()
      if (data.ok) {
        setResult(`Copied: ${data.copied.sections} sections, ${data.copied.topics} topics`)
      } else {
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="copySections" className="text-sm">Exam Sections (structure, question types)</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox id="copyTopics" checked={copyTopics} onCheckedChange={v => setCopyTopics(!!v)} />
                <Label htmlFor="copyTopics" className="text-sm">Topics</Label>
              </div>
            </div>
